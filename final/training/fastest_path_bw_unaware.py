...
def plot_final_graph(min_epochs, min_costs, start_state, end_state, epoch):
    ...
    plt.title('Minimum Time vs Epoch (Without bandwith)')
    plt.savefig(f"epoch_vs_cost_plots/fastest_path_without_bandwidth_image_epoch_{start_state}_{end_state}_{epoch}.png")
...
def q_learning(df, total_data_to_be_transfer, D, node_indices_with_x_y, num_nodes, start_state, end_state, num_epoch, gamma, epsilon_start, epsilon_end, alpha, save_interval):
    ...
    shortest_path_exploit_epochs = int(0.50 * num_epoch)  # 20% of num_epoch
    
    # Flag to indicate whether the agent is currently exploring or exploiting
    exploring = True
    exploit_epochs = 0  # Number of epochs the agent has been exploiting the shortest path
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        while s_cur != end_state:
            ...
            reward = -(D[s_cur][s_next])*(traffic_dens)/band_width # + data_transfer_reward
        ...
        # Exploit shortest path for a certain number of epochs after finding it
        if exploring:
            if epoch >= shortest_path_exploit_epochs:
                exploring = False
        else:
            if exploit_epochs < shortest_path_exploit_epochs:
                path = final_path[:]  # Exploit the shortest path
                exploit_epochs += 1
            else:
                exploring = True
                exploit_epochs = 0  # Reset exploit_epochs counter
...
if __name__ == '__main__':
    ...
    end_state = 25
    num_epoch = 20000
    total_data_to_be_transfer = 10    # In Bits
    user_id = 1455
    ...
    for run in range(10):  # Adjust the number of runs as needed

        wandb.init(project="new_test1")
        config = wandb.config
        config.Agent = "BW-Unaware"
